...
function calculate_scc_base_mc(model::Model, trials::Int64, persist_dist::Bool, emuc_dist::Bool, prtp_dist::Bool, pulse_year::Int64, pulse_size::Float64, emuc::Float64; eta_AMOC_dist_scc::Bool=false, mean_eta_AMOC_scc::Union{Float64, Nothing}=nothing, std_eta_AMOC_scc::Union{Float64, Nothing}=nothing, beta_AMOC_dist_scc::Bool=false, std_beta_AMOC_scc::Union{Float64, Nothing}=nothing, AMOC_pi_dist_scc::Bool=false, std_AMOC_pi_scc::Union{Float64, Nothing}=nothing, calc_nationals::Bool=true, sample_id_subset::Union{Vector, Nothing}=nothing)
    update_param!(model, :Utility_EMUC, emuc); run(model)
    mm = calculate_scc_setup(model, pulse_year, pulse_size)
    ...
    sim_base(mm, trials, persist_dist, emuc_dist, prtp_dist; 
             ...
             beta_AMOC_dist=beta_AMOC_dist_scc, std_beta_AMOC=std_beta_AMOC_scc,
